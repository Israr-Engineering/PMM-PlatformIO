...
static const uint8_t PMM_DO_Pins[4] = {10ul,11ul,12ul,13ul};
static const uint8_t ETHERRST = 24ul;
static const uint8_t DIPPROG01 = 46ul ;
static const uint8_t DIPPROG02 = 47ul  ;
static const uint8_t DILOSSPOWER = 16ul  ;
static const uint8_t DIFACTORYRESET = 23ul  ;
...
void ThisProductSetup()
{
    ...
    // Output Pins
    for (uint8_t i = 0; i < 4; i++) pinMode(PMM_DO_Pins[i], OUTPUT);
    for (uint8_t i = 0; i < 4; i++) digitalWrite(PMM_DO_Pins[i], LOW);
