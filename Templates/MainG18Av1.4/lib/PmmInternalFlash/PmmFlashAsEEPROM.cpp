...
PmmEEPROMClass::PmmEEPROMClass(void) : _initialized(false), _dirty(false) {
  // Empty
}

uint8_t PmmEEPROMClass::read(int address)
{
  if (!_initialized) init();
  return _eeprom.data[address];
}

void PmmEEPROMClass::update(int address, uint8_t value)
{
  if (!_initialized) init();
...
void PmmEEPROMClass::write(int address, uint8_t value)
{
  update(address, value);
}

void PmmEEPROMClass::init()
{
...
bool PmmEEPROMClass::isValid()
{
  if (!_initialized) init();
  return _eeprom.valid;
}

void PmmEEPROMClass::commit()
{
  if (!_initialized) init();
  if (_dirty) {
    _eeprom.valid = true;
