...
void PmmModBus::ModbusServersUpdate(int serial)
{
    ...
    for (int i = 0; i < 120; i++)
    {
        ...
        if (PmmIO.Inputs[i] != PmmIO.InputsPrev[i])
        {
            ...
            if (ThisProduct.ModbusRTUServerRunning) PMMModBUSRTUServerinputRegisterWrite(i,PmmIO.Inputs[i]);
        ...
        if (PmmIO.Outputs[i] != PmmIO.OutputsPrev[i])
        {
            ...
            if (ThisProduct.ModbusRTUServerRunning) PMMModBUSRTUServerholdingRegisterWrite(i,PmmIO.Outputs[i]);
            if (ThisProduct.ModbusTCPServerRunning) PMMModBUSRTUServerholdingRegisterWrite(i,PmmIO.Outputs[i]);
            
        }
        // Read to modbus server
            if (ThisProduct.ModbusRTUServerRunning) PmmIO.Outputs[i] = PMMModBUSRTUServerholdingRegisterReadOneValue(i);
        ...
        if (PmmIO.InputsByte[i] != PmmIO.InputsBytePrev[i])
        {
            ...
            if (ThisProduct.ModbusRTUServerRunning) PMMModBUSRTUServerdiscreteInputWrite(i,PmmIO.InputsByte[i]);
        ...
        if (ThisProduct.ModbusRTUServerRunning) PmmIO.Outputs[i] = PMMModBUSRTUServercoilReadOneValue(i);
            if (ThisProduct.ModbusTCPServerRunning) PmmIO.Outputs[i] = PMMmodbusTCPServercoilRead(i);
