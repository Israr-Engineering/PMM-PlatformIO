 ...
 *          Wire object
 */
PmmBNO055::PmmBNO055(int32_t sensorID, uint8_t address,
                                 TwoWire *theWire) {
 ...
 */
bool PmmBNO055::begin(adafruit_bno055_opmode_t mode) {
 ...
 *            OPERATION_MODE_NDOF]
 */
void PmmBNO055::setMode(adafruit_bno055_opmode_t mode) {
  _mode = mode;
 ...
 */
adafruit_bno055_opmode_t PmmBNO055::getMode() {
 ...
 *           REMAP_SIGN_P7]
 */
void PmmBNO055::setAxisSign(adafruit_bno055_axis_remap_sign_t remapsign) {
  adafruit_bno055_opmode_t modeback = _mode;
 ...
 */
void PmmBNO055::setExtCrystalUse(boolean usextal) {
  adafruit_bno055_opmode_t modeback = _mode;
 ...
 *           system error info
 */
void PmmBNO055::getSystemStatus(uint8_t *system_status,
 ...
 *          revision info
 */
void PmmBNO055::getRevInfo(adafruit_bno055_rev_info_t *info) {
  uint8_t a, b;
 ...
 */
void PmmBNO055::getCalibration(uint8_t *sys, uint8_t *gyro,
 ...
 */
imu::Vector<3> PmmBNO055::getVector(adafruit_vector_type_t vector_type) {
  imu::Vector<3> xyz;
  uint8_t buffer[6];
  memset(buffer, 0, 6);
 ...
 */
imu::Quaternion PmmBNO055::getQuat() {
  uint8_t buffer[8];
  memset(buffer, 0, 8);
 ...
 *          Sensor description
 */
void PmmBNO055::getSensor(sensor_t *sensor) {
 ...
 */
bool PmmBNO055::getEvent(sensors_event_t *event) {
  /* Clear the event */
  ...
  imu::Vector<3> euler = getVector(PmmBNO055::VECTOR_EULER);
...
bool PmmBNO055::getEvent(sensors_event_t *event,
  ...
  imu::Vector<3> vec;
  if (vec_type == PmmBNO055::VECTOR_LINEARACCEL) {
    event->type = SENSOR_TYPE_LINEAR_ACCELERATION;
    vec = getVector(PmmBNO055::VECTOR_LINEARACCEL);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == PmmBNO055::VECTOR_ACCELEROMETER) {
    event->type = SENSOR_TYPE_ACCELEROMETER;
    vec = getVector(PmmBNO055::VECTOR_ACCELEROMETER);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == PmmBNO055::VECTOR_GRAVITY) {
    event->type = SENSOR_TYPE_GRAVITY;
    vec = getVector(PmmBNO055::VECTOR_GRAVITY);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == PmmBNO055::VECTOR_EULER) {
    event->type = SENSOR_TYPE_ORIENTATION;
    vec = getVector(PmmBNO055::VECTOR_EULER);

    event->orientation.x = vec.x();
    event->orientation.y = vec.y();
    event->orientation.z = vec.z();
  } else if (vec_type == PmmBNO055::VECTOR_GYROSCOPE) {
    event->type = SENSOR_TYPE_GYROSCOPE;
    vec = getVector(PmmBNO055::VECTOR_GYROSCOPE);

    event->gyro.x = vec.x() * SENSORS_DPS_TO_RADS;
    event->gyro.y = vec.y() * SENSORS_DPS_TO_RADS;
    event->gyro.z = vec.z() * SENSORS_DPS_TO_RADS;
  } else if (vec_type == PmmBNO055::VECTOR_MAGNETOMETER) {
    event->type = SENSOR_TYPE_MAGNETIC_FIELD;
    vec = getVector(PmmBNO055::VECTOR_MAGNETOMETER);
 ...
 */
bool PmmBNO055::getSensorOffsets(uint8_t *calibData) {
 ...
 *  @return true if read is successful
 */
bool PmmBNO055::getSensorOffsets(
 ...
 *          calibration data
 */
void PmmBNO055::setSensorOffsets(const uint8_t *calibData) {
  adafruit_bno055_opmode_t lastMode = _mode;
  setMode(OPERATION_MODE_CONFIG);
  delay(25);
 ...
 */
void PmmBNO055::setSensorOffsets(
 ...
 */
bool PmmBNO055::isFullyCalibrated() {
 ...
 */
void PmmBNO055::enterSuspendMode() {
  adafruit_bno055_opmode_t modeback = _mode;
 ...
 */
void PmmBNO055::enterNormalMode() {
  adafruit_bno055_opmode_t modeback = _mode;
 ...
 */
bool PmmBNO055::write8(adafruit_bno055_reg_t reg, byte value) {
 ...
 */
byte PmmBNO055::read8(adafruit_bno055_reg_t reg) {
 ...
 */
bool PmmBNO055::readLen(adafruit_bno055_reg_t reg, byte *buffer,
                              uint8_t len) {
