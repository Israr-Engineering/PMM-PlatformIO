...
void GenerateAlarm()
{
    ...
    else // Slave
    {
        ...
        if ( int_output[2] == 1967)
        {
            // int_input[2]++;
            // if (int_input[3] > 1967)
            //     int_input[3] = 1948; // JUST DONT FORGET THE DATES
...
void syncModbusBuffers()
{
    ...
    uint16_t port = 502;
    //if (ModbusTCPMode) ModbusTCPServer.PmmModBusTCPClientSetup(SlaveIP,  port);
    ...
    else // Slave Mode
    {
        int_input[2] = 1948 ;
        int_output[2] = 0 ;
         //if (ModbusTCPMode) ModbusTCPServer.PmmModBusTCPServerSetup(502, Pmm1003ID);
