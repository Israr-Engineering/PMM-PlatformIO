...
bool PmmExternalEEPROM::begin(uint8_t deviceAddress, TwoWire &wirePort)
{
...
//Erase entire EEPROM
void PmmExternalEEPROM::erase(uint8_t toWrite)
{
...
uint32_t PmmExternalEEPROM::length()
{
  return settings.memorySize_bytes;
}

//Returns true if device is detected
bool PmmExternalEEPROM::isConnected(uint8_t i2cAddress)
{
  if (i2cAddress == 255)
...
bool PmmExternalEEPROM::isBusy(uint8_t i2cAddress)
{
  if (i2cAddress == 255)
    i2cAddress = settings.deviceAddress; //We can't set the default to settings.deviceAddress so we use 255 instead
...
//}
void PmmExternalEEPROM::setMemorySize(uint32_t memSize)
{
  settings.memorySize_bytes = memSize;
}
uint32_t PmmExternalEEPROM::getMemorySize()
{
  return settings.memorySize_bytes;
}
void PmmExternalEEPROM::setPageSize(uint16_t pageSize)
{
  settings.pageSize_bytes = pageSize;
}
uint16_t PmmExternalEEPROM::getPageSize()
{
  return settings.pageSize_bytes;
}
void PmmExternalEEPROM::setPageWriteTime(uint8_t writeTimeMS)
{
  settings.pageWriteTime_ms = writeTimeMS;
}
uint8_t PmmExternalEEPROM::getPageWriteTime()
{
  return settings.pageWriteTime_ms;
}
void PmmExternalEEPROM::enablePollForWriteComplete()
{
  settings.pollForWriteComplete = true;
}
void PmmExternalEEPROM::disablePollForWriteComplete()
{
  settings.pollForWriteComplete = false;
}
constexpr uint16_t PmmExternalEEPROM::getI2CBufferSize()
{
  return I2C_BUFFER_LENGTH_TX;
}
//Read a byte from a given location
uint8_t PmmExternalEEPROM::read(uint32_t eepromLocation)
{
  uint8_t tempByte;
...
void PmmExternalEEPROM::read(uint32_t eepromLocation, uint8_t *buff, uint16_t bufferSize)
{
  uint16_t received = 0;
...
void PmmExternalEEPROM::write(uint32_t eepromLocation, uint8_t dataToWrite)
{
...
void PmmExternalEEPROM::write(uint32_t eepromLocation, const uint8_t *dataToWrite, uint16_t bufferSize)
{
  //Error check
