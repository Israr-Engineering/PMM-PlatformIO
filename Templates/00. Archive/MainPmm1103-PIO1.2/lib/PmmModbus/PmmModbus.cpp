...
PmmEthernetClient ethClient; // PmmModbusTCPClient
...
int PmmModBus::PmmModBusTCPClientSetup(IPAddress ip, uint16_t port)
{
    int result = 0 ;
        if (!modbusTCPClient.connected())
        {
            //SerialUSB.println("Reconnect");
            modbusTCPClient.setTimeout(500);
           result=  modbusTCPClient.begin(ip, port);
        }
...
int PmmModBus::PmmModBusTCPClientCoilRead(/*IPAddress ip, uint16_t port,*/ int SlaveID, int address)
{
    int value = 0;
    ...
    if (modbusTCPClient.connected())
    {
        value = modbusTCPClient.coilRead(SlaveID, address);
    }

    return value;
}

int PmmModBus::PmmModBusTCPClientDiscreteRead(/*IPAddress ip, uint16_t port,*/ int SlaveID, int address)
{
    long value = 0;
    ...
    if (modbusTCPClient.connected())
    {
        value = modbusTCPClient.discreteInputRead(SlaveID, address);
    }

    return value;
}

long PmmModBus::PmmModBusTCPClientHoldingRead(/*IPAddress ip, uint16_t port,*/ int SlaveID, int address)
{
    long value = 0;
    ...
    if (modbusTCPClient.connected())
    {
        value = modbusTCPClient.holdingRegisterRead(SlaveID, address);
    }

    return value;
}

long PmmModBus::PmmModBusTCPClientInputRead(/*IPAddress ip, uint16_t port,*/ int SlaveID, int address)
{
    long value = 0;
    ...
    if (modbusTCPClient.connected())
    {
        value = modbusTCPClient.inputRegisterRead(SlaveID, address);
    }

    return value;
}

void PmmModBus::PmmModBusTCPClientCoilWrite(/*IPAddress ip, uint16_t port,*/ int SlaveID, int address, uint8_t value)
{
    ...
    if (modbusTCPClient.connected())
    {
        modbusTCPClient.coilWrite(SlaveID, address, value);
        
    }
}

void PmmModBus::PmmModBusTCPClientHoldingWrite(/*IPAddress ip, uint16_t port,*/ int SlaveID, int address, uint16_t value)
{
    ...
    if (modbusTCPClient.connected())
    {
        modbusTCPClient.holdingRegisterWrite(SlaveID, address, value);
    }
}
