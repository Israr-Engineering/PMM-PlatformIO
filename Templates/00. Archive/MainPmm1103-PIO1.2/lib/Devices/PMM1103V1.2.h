...
void hardwareInit()
{
    ...
    MappingRegisters();
    ModbusRTU01Server.PmmModBusRTUServerHoldingWrite(3, 1967); // update holding registers from input reg
...
void updateOutputBuffers()
{
    ...
    // 2. check max-min limit tolerances
    ...
    // Update other things
    // 5. check max-min limit tolerances again
    if (Pmmm1103.MPUCalAngle > Pmmm1103.MAXAngle)
        Pmmm1103.EastEnable = false; // block East direction
    if (Pmmm1103.MPUCalAngle < Pmmm1103.MINAngle)
        Pmmm1103.WestEnable = false; // block west direction
...
void RunAutoMode()
{
    ...
    {
        // ModbusRTU01Server.PmmModBusRTUServerHoldingWrite(24, int(Pmmm1103.MAXAngle * 100));
        Pmmm1103.RemoteTargetAngle = Pmmm1103.MAXAngle;
    }
    if (Pmmm1103.RemoteTargetAngle < Pmmm1103.MINAngle)
    {
        // ModbusRTU01Server.PmmModBusRTUServerHoldingWrite(24, int(Pmmm1103.MINAngle * 100));
...
void MappingRegisters()
{
    ...
    bool_input[0][7] = Pmmm1103.IsNotFirstStart;
    bool_input[1][0] = Pmmm1103.MoveEast;
    bool_input[1][1] = Pmmm1103.MoveWest;
    // Realtime IO
    ...
    // STEP04 : Commands
    // 1. // Generate Save to ROM Command
    if (int_output[3] == 820)
    {
    ...
    // 2. // Copy ROM TO Holding registers
