...
PmmModbusServer::PmmModbusServer() :
  _mb(NULL)
{
...
int PmmModbusServer::configureCoils(int startAddress, int nb)
{
...
int PmmModbusServer::configureDiscreteInputs(int startAddress, int nb)
{
  if (startAddress < 0 || nb < 1) {
    errno = EINVAL;
...
int PmmModbusServer::configureHoldingRegisters(int startAddress, int nb)
{
  if (startAddress < 0 || nb < 1) {
    errno = EINVAL;
...
int PmmModbusServer::configureInputRegisters(int startAddress, int nb)
{
  if (startAddress < 0 || nb < 1) {
    errno = EINVAL;
...
int PmmModbusServer::coilRead(int address)
{
...
int PmmModbusServer::discreteInputRead(int address)
{
...
long PmmModbusServer::holdingRegisterRead(int address)
{
...
long PmmModbusServer::inputRegisterRead(int address)
{
...
int PmmModbusServer::coilWrite(int address, uint8_t value)
{
  if (_mbMapping.start_bits > address ||
      (_mbMapping.start_bits + _mbMapping.nb_bits) < (address + 1)) {
    errno = EMBXILADD;
...
int PmmModbusServer::holdingRegisterWrite(int address, uint16_t value)
{
  if (_mbMapping.start_registers > address || 
      (_mbMapping.start_registers + _mbMapping.nb_registers) < (address + 1)) {
    errno = EMBXILADD;
...
int PmmModbusServer::registerMaskWrite(int address, uint16_t andMask, uint16_t orMask)
{
...
int PmmModbusServer::discreteInputWrite(int address, uint8_t value)
{
  return writeDiscreteInputs(address, &value, 1);
}

int PmmModbusServer::writeDiscreteInputs(int address, uint8_t values[], int nb)
{
  if (_mbMapping.start_input_bits > address || 
...
int PmmModbusServer::inputRegisterWrite(int address, uint16_t value)
{
  return writeInputRegisters(address, &value, 1);
}

int PmmModbusServer::writeInputRegisters(int address, uint16_t values[], int nb)
{
  if (_mbMapping.start_input_registers > address || 
...
int PmmModbusServer::begin(modbus_t* mb, int id)
{
  end();
