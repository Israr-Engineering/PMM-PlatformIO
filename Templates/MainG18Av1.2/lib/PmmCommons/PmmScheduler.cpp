...
PmmSchedulerClass::PmmSchedulerClass() {
	coopInit();
}
...
void PmmSchedulerClass::startLoop(SchedulerTask task, uint32_t stackSize) {
...
void PmmSchedulerClass::start(SchedulerTask task, uint32_t stackSize) {
	coopSpawn(startTaskHelper, reinterpret_cast<void *>(task), stackSize);
}

void PmmSchedulerClass::start(SchedulerParametricTask task, void *taskData, uint32_t stackSize) {
	coopSpawn(task, taskData, stackSize);
}

PmmSchedulerClass Scheduler;
