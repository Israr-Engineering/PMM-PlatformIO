...
void PmmModBus::ModbusServersUpdate(int serial)
{
    ...
    for (int i = 0; i < 120; i++)
    {
        ...
        if (PmmIO.InputsByte[i] != PmmIO.InputsBytePrev[i])
        {
            ...
            if (ThisProduct.PmmSerial[serial].PmmProtocols.IsRunning) PMMModBUSRTUServerdiscreteInputWrite(i,PmmIO.InputsByte[i]);
        ...
        if (ThisProduct.PmmSerial[serial].PmmProtocols.IsRunning) PmmIO.Outputs[i] = PMMModBUSRTUServercoilReadOneValue(i);
